...
function normalizeParallelGroupKey(value) {
...
function normalizeGeneratorData(
    ...
    const normalized = {
        ...
        placedTasks:
            Array.isArray(data?.placedTasks)
                ? [...data.placedTasks]
                : [],
    ...
    if (
        ...
        data.requirements.forEach(
            requirement => {
                ...
                if (
                    !Number.isFinite(
                        Number(requirement.lessonsPerWeek)
                    ) ||
                    Number(requirement.lessonsPerWeek) <= 0
                ) {
                ...
                if (
                    !Number.isFinite(
                        Number(requirement.doubleLessonsPerWeek)
                    ) ||
                    Number(requirement.doubleLessonsPerWeek) < 0
                ) {
                ...
                if (
                    ...
                    Number(requirement.doubleLessonsPerWeek) >
                    Math.floor(
                        Number(requirement.lessonsPerWeek) / 2
                    )
                ) {
                ...
                if (
                    !Number.isFinite(
                        Number(requirement.maxLessonsPerDay)
                    ) ||
                    Number(requirement.maxLessonsPerDay) <= 0
                ) {
...
function validateTimetablePeriods(data) {
    ...
    data.periods.forEach(
        period => {
            ...
            if (
                !Number.isFinite(
                    Number(period?.periodOrder)
                ) ||
                Number(period.periodOrder) <= 0
            ) {
...
function createLessonTasks(
    ...
    // ========================================================
    // BUILD ACTUAL SCHOOL-WIDE PARALLEL GROUP MAP
    // ========================================================
    //
    // IMPORTANT:
    //
    // A parallel group contains ONLY the requirements that
    // actually exist in that group.
    //
    // We NEVER create missing requirements for other streams.
    //
    // There are two valid structures:
    //
    // 1. MULTI-STREAM PARALLEL GROUP
    //
    //    Example:
    //        RE/GE/BS
    //
    //    10L -> 5 actual requirements
    //    10M -> 5 actual requirements
    //    10T -> 5 actual requirements
    //
    //    Each stream may have only ONE group requirement at a
    //    given occurrence.
    //
    //    Therefore:
    //
    //        O1-O5   -> requirement 1 in each stream
    //        O6-O10  -> requirement 2 in each stream
    //        ...
    //        O21-O25 -> requirement 5 in each stream
    //
    // 2. SINGLE-STREAM PARALLEL GROUP
    //
    //    Example:
    //        BIO/PHY -> 10E
    //
    //    Both actual requirements may share O1-O5.
    //
    //    This allows:
    //
    //        Biology O1 + Physics O1
    //
    //    in the same stream and same period.
    //
    // ========================================================
    ...
    // CRITICAL FIX:
    //
    // We no longer assume that every requirement in a
    // multi-stream group can use O1-O5.
    //
    // For MULTI-STREAM groups:
    //
    //     each stream receives its requirements sequentially.
    //
    // Example:
    //
    //     10L:
    ...
    //         ...
    //
    // Thus each occurrence contains at most ONE group lesson
    // for each participating stream.
    //
    // For SINGLE-STREAM groups:
    //
    //     all actual requirements share the same occurrence
    //     sequence.
    //
    // Example BIO/PHY:
    //
    ...
    parallelRequirementMap.forEach(
        (
            ...
            // ------------------------------------------------
            // Determine actual participating streams.
            // ------------------------------------------------

            const actualStreamIds =
                groupInfo.streamIds instanceof Set
                    ? [
                        ...groupInfo.streamIds
                    ].map(
                        id =>
            ...
            const isSingleStreamGroup =
                actualStreamIds.length <= 1;


            // ------------------------------------------------
            // Map requirement -> occurrence indexes.
            //
            // Each requirement receives an explicit array:
            //
            //     [1,2,3,4,5]
            //
            // or:
            ...
            // =================================================
            //
            // Each stream gets its own sequential occurrence
            // lane.
            //
            // This prevents:
            //
            ...
            //     10L Req2 O1
            //
            // from becoming two simultaneous lessons for 10L.
            //
            // Instead:
            //
            //     Req1 -> O1-O5
            //     Req2 -> O6-O10
            //
            // while another participating stream follows the
            // same occurrence positions.
            //
            // =================================================
                ...
                // ------------------------------------------------
                // Process each ACTUAL participating stream only.
                // ------------------------------------------------
                ...
                requirementsByStream.forEach(
                    streamRequirements => {
                        ...
                        //
                        // Preserve the supplied group order as
                        // the primary order, with requirementId
                        // only as a deterministic fallback.
                        // ----------------------------------------

                        streamRequirements.sort(
                            (
                                a,
                                b
                            ) => {
                ...
                groupRequirements.map(
                    requirement => ({
                        ...
                        occurrenceIndexes:
                            [
                                ...(
                                    requirementOccurrenceAssignments.get(
                                        String(
                                            requirement?.requirementId ??
                                            requirement?.id ??
                                            ""
            ...
            plan => {
                ...
                console.log(
                    `Parallel group ${plan.groupKey}:`,
                    {
                        ...
                        mode:
                            plan.isSingleStreamGroup
                                ? "SINGLE-STREAM SHARED OCCURRENCES"
                                : "MULTI-STREAM OCCURRENCE LANES",
    ...
    requirements.forEach(
        requirement => {
            ...
            // =================================================
            // GET THIS REQUIREMENT'S GROUP OCCURRENCES
            // =================================================
            //
            // This is the key correction.
            //
            // The occurrence indexes are assigned by the GROUP
            // PLAN, not independently starting from O1 for every
            // requirement.
            //
            // =================================================

            const requirementOccurrenceIndexes =
                isParallel
            ...
            if (
                isParallel &&
                requirementOccurrenceIndexes.length !==
                lessonsPerWeek
            ) {

                throw new Error(

                    `Parallel occurrence assignment mismatch for ` +
                    ...
                    `expected ${lessonsPerWeek} occurrence indexes ` +
            ...
            for (
                ...
                const occurrenceStart =
                    isParallel
                        ...
                        ? requirementOccurrenceIndexes[
                            occurrenceArrayIndex
                        ]
                ...
                if (
                    ...
                    throw new Error(
                        ...
                        `could not obtain two consecutive ` +
                ...
                if (
                    ...
                    throw new Error(
                        ...
                        `Parallel double occurrence is not consecutive for ` +
            ...
            for (
                ...
                if (
                    ...
                    throw new Error(
                        ...
                        `could not obtain its occurrence index.`
...
function createLessonTasks(
    ...
    if (!Array.isArray(requirements)) {
        throw new Error(
            "createLessonTasks: requirements must be an array."
        );
    }

    if (!lookup || typeof lookup !== "object") {
        throw new Error(
            "createLessonTasks: lookup maps are required."
        );
    }

    if (!(lookup.streams instanceof Map)) {
        throw new Error(
            "createLessonTasks: lookup.streams must be a Map."
        );
    }

    if (!(lookup.subjects instanceof Map)) {
        throw new Error(
            "createLessonTasks: lookup.subjects must be a Map."
        );
    }

    if (!(lookup.teachers instanceof Map)) {
        throw new Error(
            "createLessonTasks: lookup.teachers must be a Map."
        );
    }
    ...
    parallelRequirementMap.forEach(
        group => {
            ...
            for (
                let occurrence = 1;
                occurrence <= maximumOccurrenceCount;
                occurrence++
            ) {
    ...
    requirements.forEach(
        requirement => {
            ...
            if (!requirementId) {
                console.warn(
                    "Skipping requirement without valid ID:",
                    requirement
                );
            ...
            if (!stream) {
                throw new Error(
                    `Requirement ${requirementId} references missing stream ${streamId}.`
                );
            }

            if (!subject) {
                throw new Error(
                    `Requirement ${requirementId} references missing subject ${subjectId}.`
                );
            }
...
function validateLessonTasks(
    ...
    // ========================================================
    // TRACK OCCURRENCES BY REQUIREMENT
    // ========================================================
    ...
    tasks.forEach(
        task => {
            ...
            if (
                ...
                errors.push(
                    "An invalid lesson task object was generated."
                );
            ...
            if (
                ...
                errors.push(
                    `Task ${task.taskId || "[unknown]"} has invalid periodIds; expected an array.`
                );
            ...
            if (
                ...
                if (
                    ...
                    errors.push(
                        `Non-parallel task ${task.taskId} has parallel occurrence indexes.`
                    );
            ...
            if (
                ...
                task.parallelGroupStreamIds !== null
            ) {
                ...
                if (
                    ...
                    errors.push(
                        `Parallel task ${task.taskId} has invalid parallel group stream IDs.`
                    );
            ...
            // =================================================
            // RECORD REQUIREMENT OCCURRENCE COVERAGE
            // =================================================
            ...
            if (
                ...
                if (
                    !requirementParallelCoverage.has(
                        requirementKey
                    )
                ) {

                    requirementParallelCoverage.set(
                        requirementKey,
                        {
                            parallelGroup:
                                parallelGroup,
                ...
                const requirementCoverage =
                    requirementParallelCoverage.get(
                        requirementKey
                    );


                if (
                    requirementCoverage.parallelGroup !==
                    parallelGroup
                ) {

                    errors.push(
                        `Requirement ${requirementKey} appears in multiple parallel groups: ` +
                        `${requirementCoverage.parallelGroup} and ${parallelGroup}.`
                    );
                ...
                if (
                    ...
                    errors.push(
                        `Requirement ${requirementKey} appears in multiple streams within parallel tasks.`
                    );
                ...
                for (
                    ...
                    requirementCoverage.occurrences.add(
                        occurrence
                    );
                ...
                requirementCoverage.tasks.push(
                    task
                );
    ...
    data.requirements.forEach(
        requirement => {
            ...
            if (
                ...
                errors.push(
                    `Requirement ${requirementId}: lessonsPerWeek must be a non-negative integer; received ${requirement.lessonsPerWeek}.`
                );
                ...
                errors.push(
                    `Requirement ${requirementId}: lessonsPerWeek cannot be negative.`
                );
    ...
    parallelGroups.forEach(
        (
                    ...
                    occurrenceInfo.streams.forEach(
                        streamId => {
                            ...
                            if (
                                ...
                                errors.push(
                                    `Parallel group ${groupKey}, ` +
                                    `${occurrenceKey}: unexpected stream ${streamId} ` +
                    ...
                    occurrenceInfo.requirements.forEach(
                        (
                            ...
                            if (
                                ...
                                errors.push(
                                    `Parallel group ${groupKey}, ` +
                                    `${occurrenceKey}: unexpected requirement ${requirementId} ` +
                                    `was assigned to the occurrence.`
                                );
                    ...
                    if (
                        ...
                        streamRequirements.forEach(
                            (
                                ...
                                if (
                                    uniqueRequirementIds.length > 1
                                ) {

                                    errors.push(
                                        `Parallel group ${groupKey}, ` +
                                        `${occurrenceKey}: stream ${streamId} ` +
                                        `has multiple requirements in the same ` +
                                        `multi-stream parallel occurrence: ` +
                                        `${uniqueRequirementIds.join(", ")}.`
                                    );
            ...
            // =================================================
            // CHECK GROUP-WIDE OCCURRENCE CONTINUITY
            // =================================================
            ...
            if (
                uniqueCoveredOccurrenceNumbers.length > 0
            ) {

                const firstCovered =
                    uniqueCoveredOccurrenceNumbers[0];
                ...
                if (
                    ...
                    errors.push(
                        `Parallel group ${groupKey}: ` +
                        `occurrence sequence starts at O${firstCovered} ` +
                        `instead of O1.`
                    );
                ...
                for (
                    ...
                    const previous =
                        uniqueCoveredOccurrenceNumbers[
                            index - 1
                        ];
            ...
            // =================================================
            // CHECK REQUIREMENT OCCURRENCE CONTINUITY
            // =================================================

            requirementParallelCoverage.forEach(
                (
                    coverage,
                    requirementId
                ) => {
                    ...
                    for (
                        ...
                        if (
                            ...
                            errors.push(
                                `Parallel group ${groupKey}: ` +
                                `requirement ${requirementId} ` +
                                `has a gap in its occurrence sequence. ` +
                                `Expected O${expected} but found O${actual}.`
                            );
                                ...
                                task => {
                                    ...
                                    if (
                                        ...
                                        errors.push(
                                            ...
                                            `contains duplicate task representation ` +
                                            `for stream ${streamId}.`
                                        );
    ...
    // ========================================================
    // VALIDATE TASK PARALLEL GROUP STREAM MEMBERSHIP
    // ========================================================
    ...
    tasks.forEach(
        task => {
            ...
            if (
                ...
                warnings.push(
                    `Parallel task ${task.taskId}: no authoritative parallel group definition was available for ${groupKey}.`
                );
            ...
            if (
                authoritativeGroup.streamIds instanceof Set &&
                authoritativeGroup.streamIds.size > 0 &&
                !authoritativeGroup.streamIds.has(
                    actualStreamId
                )
            ) {
                ...
                errors.push(
                    `Parallel task ${task.taskId}: ` +
                    `stream ${actualStreamId} does not belong to ` +
                    `parallel group ${groupKey}.`
                );
...
function getCompatibleRooms(
    ...
    //     requiresRoom
    //
    // roomRequired is the current canonical property used
    // by createLessonTasks().
    // ========================================================

    const roomRequired =
        task?.roomRequired === true ||
...
function createOccupancyIndexes(
    ...
    const occupancy = {
        ...
        // ====================================================
        // DETAILED STREAM/PERIOD LESSONS
        // ====================================================
        ...
        // ====================================================
        // DETAILED STUDENT GROUP/PERIOD LESSONS
        // ====================================================
    ...
    entries.forEach(
        entry => {
            ...
            if (
                ...
                occupancy.teacherPeriodLessons
                    ...
                    .push({
                        ...
                        requirementId:
                            lessonDetail.requirementId,

                        subjectId:
                            lessonDetail.subjectId,

                        streamId:
                            lessonDetail.streamId,

                        parallelGroup:
                            lessonDetail.parallelGroup,

                        parallelOccurrenceKey:
                            lessonDetail.parallelOccurrenceKey,

                        parallelOccurrence:
                            lessonDetail.parallelOccurrence,

                        parallelOccurrenceIndexes:
                            [
                                ...lessonDetail.parallelOccurrenceIndexes
                            ],

                        parallelIdentity:
                            lessonDetail.parallelIdentity,

                        studentGroupIds:
                            [
                                ...lessonDetail.studentGroupIds
                            ]
            ...
            if (
                ...
                // ---------------------------------------------
                // STORE DETAILED STREAM/PERIOD LESSON
                // ---------------------------------------------

                if (
                    !occupancy.streamPeriodLessons.has(
                        streamPeriodKey
                    )
                ) {

                    occupancy.streamPeriodLessons.set(
                        streamPeriodKey,
                        []
                    );
    ...
    entries.forEach(
        entry => {
            ...
            const uniqueLessonId =
                ...
                (
                    parallelOccurrenceKey
                        ? `${requirementId}__${parallelOccurrenceKey}`
...
function getTeacherMaxConsecutiveLessons(
    ...
    const maximum =
        Number(
            ...
            limits.max_consecutive_lessons
        );
...
function getIndexedPeriod(
    ...
    // ========================================================
    // FAST LOOKUP WHEN PERIOD MAP EXISTS
    // ========================================================

    if (
        indexes.periodLookup instanceof Map
    ) {

        return (
            indexes.periodLookup.get(
                normalizedPeriodId
            ) ||
            null
        );
...
function getTaskStudentGroups(
    ...
    if (
        ...
        const normalized =
            normalizeTimetableId(
                typeof possibleGroups === "object"
                    ? (
                        possibleGroups.id ??
                        possibleGroups.groupId ??
                        possibleGroups.group_id ??
                        possibleGroups.streamId ??
                        possibleGroups.stream_id
                    )
                    : possibleGroups
            );
...
// ============================================================
// CHECK WHETHER TWO TEACHER LESSONS MAY SHARE A SESSION
// ============================================================
...
function areConcurrentTeacherLessonsAllowed(
    ...
    // same subject is sufficient here because the caller has
    // already matched the same teacher's occupancy index.
        ...
        const parallelTeachingAllowed =
            existingLessons.every(
                existingLesson => {
                    ...
                    // ------------------------------------------------
                    // Same subject in the same student group cannot
                    // occupy the same period unless it is explicitly
                    // represented as the same parallel session.
                    // ------------------------------------------------
                    ...
                    // ------------------------------------------------
                    // Different teachers cannot teach the same
                    // student group simultaneously.
                    // ------------------------------------------------
                    ...
                    // ------------------------------------------------
                    // Different subjects may share a period only when
                    // they belong to the SAME explicit parallel group
                    // and SAME occurrence.
                    // ------------------------------------------------
...
function reserveSlot(
    ...
    // ========================================================
    //
    // A double lesson occupying two periods must count as ONE
    // lesson for the requirement/day limit.
    //
    // Use the same fallback identity as createOccupancyIndexes()
    // so live reservation and rebuilt indexes behave identically.
    //
    // ========================================================
...
function sortLessonTasksByPriority(
    ...
    sorted.sort(
        (
            ...
            const requiresRoomA =
                taskA?.roomRequired === true ||
                taskA?.requiresRoom === true;


            const requiresRoomB =
                taskB?.roomRequired === true ||
                taskB?.requiresRoom === true;
...
function calculateCandidateSlotScore(
    ...
    //
    // A matching occurrence in ANY participating stream
    // makes this period attractive.
    //
    // IMPORTANT:
    //
    ...
    //     same occurrence key
    //
    // Matching lessons are counted uniquely so that duplicate
    // index entries cannot multiply the +1000 bonus.
    //
    // ========================================================

    const matchingParallelLessons =
        new Set();
            ...
            lessons => {
                ...
                lessons.forEach(
                    existingLesson => {
                        ...
                        // ------------------------------------------------
                        // Build a stable identity for this matching lesson.
                        // ------------------------------------------------
                        ...
                        const lessonIdentity =
                            ...
                            [
                                ...
                                existingPeriodId,
                                existingParallelIdentity ||
                                    `${taskParallelGroup}__${taskParallelOccurrenceKey}`
                            ].join("__");


                        matchingParallelLessons.add(
                            lessonIdentity
                        );
    ...
    const parallelGroupMatches =
        matchingParallelLessons.size;
...
function calculateDoubleLessonCandidateScore(
    ...
    // A double occupies two timetable periods.
    ...
    if (
        ...
        if (
            ...
            // IMPORTANT:
            //
            // getTeacherWeeklyLessonCount() currently counts
            // unique teacher sessions by period + subject.
            //
            // Therefore a normal double lesson occupying two
            // periods contributes TWO weekly sessions.
            //
            // Concurrent/shared teaching contributes zero
            // additional sessions for that period.
            // ------------------------------------------------
...
function getScoredDoubleLessonCandidates(
    ...
    // ========================================================
    // HELPER:
    // SAME EXPLICIT PARALLEL OCCURRENCE
    // ========================================================
    ...
    // ========================================================
    // HELPER:
    // FIND SAME PARALLEL OCCURRENCE AT PERIOD
    // ========================================================
    ...
    // ========================================================
    // HELPER:
    // PARALLEL PAIR SYNCHRONIZATION STATE
    // ========================================================
    ...
    pairs.forEach(
        pair => {
            ...
            if (
                ...
                // If synchronization for this occurrence has
                // already started in only one of the two
                // periods, do not create a mismatched pair.
                //
                // Either both periods match the established
                // synchronized pair or neither does.
...
function releaseReservedSlot(
    ...
    if (
        ...
        if (value instanceof Set) {
            ...
            if (filtered.length > 0) {
                ...
                indexes.teacherSubjectPeriod.set(
                    key,
                    filtered
                );
    ...
    // ========================================================
    //
    // This index represents LESSONS per day, not physical
    // periods.
    //
    // A double lesson must therefore not be decremented twice.
    //
    // Stage 7 currently moves single lessons, so one release
    // corresponds to one lesson occurrence.
    //
    // ========================================================
    ...
    // ========================================================
    // REMOVE DAILY REQUIREMENT LESSON IDENTITY
    // ========================================================
    //
    // reserveSlot() creates a unique lesson/day key here.
    //
    // That key MUST be removed when the lesson is released,
    // otherwise Stage 7 relocation leaves a stale daily count.
    //
    // ========================================================
    ...
    if (
        ...
        if (
            ...
            // ------------------------------------------------
            // Only decrement the count if this exact lesson
            // identity was actually present.
            // ------------------------------------------------
    ...
    if (
        ...
        if (value instanceof Set) {
            ...
            if (filtered.length > 0) {
                ...
                indexes.parallelOccurrence.set(
                    key,
                    filtered
                );
...
//
// Therefore remainingLessons is based on lesson occurrences,
// not timetable periods.
// ============================================================
...
function getTaskDayPressure(
    ...
    // ========================================================
    //
    // Count TASKS, not timetable periods.
    //
    // single = 1 occurrence
    // double = 1 occurrence
    //
    ...
    // IMPORTANT:
    //
    // Use the indexes supplied directly to this function.
    // Do not expect data.indexes or data.placementIndexes.
    //
    // ========================================================
...
//     parallelOccurrenceKey
//
// must match an already placed lesson.
// ============================================================
...
function selectNextSmartTask(
    ...
    taskCandidates.sort(
        (
            ...
            const roomRequiredA =
                a.task?.roomRequired === true ||
                a.task?.requiresRoom === true;


            const roomRequiredB =
                b.task?.roomRequired === true ||
                b.task?.requiresRoom === true;
...
// consecutive RUN OF TEACHING PERIODS.
//
// Breaks, lunch, and non-teaching periods must NOT be counted
// as part of a consecutive teaching run.
//
// ============================================================
...
function auditTeacherConsecutiveLimits(
    ...
    entries.forEach(
        entry => {
            ...
            // STORE PERIOD BY ID
            //
            // Using a Map prevents duplicate timetable entries
            // for the same teacher / period from artificially
            // increasing the consecutive run.
            // ------------------------------------------------
    ...
    teacherDays.forEach(
        (
            ...
            // 0 or negative means no configured limit.
            if (
                maximum <= 0
            ) {
            ...
            if (
                ...
                addTimetableAuditError(
                    audit,
                    "teacherConsecutiveLimits",
                    "Teacher exceeds maximum consecutive teaching lessons.",
                    {
                        teacherId,
...
function buildStage7RoomCandidates(
    ...
    // ========================================================
    //
    // roomRequired is the canonical task-level field.
    //
    // requiresRoom / requires_room are retained only as
...
function findAlternativeSlotForExistingTask(
    ...
    // ========================================================
    //
    // The existing task was only temporarily released while
    // searching.
    //
    // It MUST always be restored before this function returns.
    // ========================================================
    ...
    if (
        ...
        if (
            ...
            // ------------------------------------------------
            // The search result cannot safely be used because
            // the original timetable state was not restored.
            // ------------------------------------------------
        ...
        // ====================================================
        //
        // No other placement should have changed the indexes
        // during this search. Therefore the original slot
        // becoming invalid indicates an unexpected occupancy
        // state.
        //
        // Attempt restoration directly because the existing
        // task was definitely released by this function.
        // ====================================================
        ...
        console.error(
            "STAGE 7: CRITICAL — original task slot became invalid while searching for relocation.",
            {
                ...
                conflictReason:
                    originalSlotConflict?.reason ||
                    "Unknown original-slot conflict."
            }
        );
        ...
        if (
            ...
            console.error(
                "STAGE 7: CRITICAL — failed to restore existing task after unexpected relocation state.",
                {
                    taskId:
                        existingTask?.taskId ??
                        existingTask?.task_id ??
        ...
        // ----------------------------------------------------
        // Never return a relocation candidate from an
        // unexpected/corrupted search state.
        // ----------------------------------------------------
